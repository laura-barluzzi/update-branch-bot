 ...
 * @typedef {import('./types').GitHubUsers} GitHubUsers
 */
...
class Github {
  ...
  async listPullRequests ({ organization, repository, pageSize = 50 }) {
    ...
    while (hasNextPage) {
      ...
      nodes.forEach(node => pullRequests.push({
        ...
        organization: node.headRepositoryOwner.login,
   ...
   * @param {string} args.organization
   * @param {number=} args.pageSize
   * @returns {Promise<GitHubUsers[]>}
   */
  async listAllUsers ({ organization, pageSize = 100 }) {
    if (this.authToken == null) {
      throw new Error('this.authToken must not be null')
    }
    ...
    const response = await got.post('https://api.github.com/graphql', {
      ...
      body: {
        query: `
          query($organization:String!, $pageSize:Int!) {
            organization(login:$organization) {
              membersWithRole(first:$pageSize){ 
                nodes {
                  login
                  email
                }
    ...
    /** @type {{nodes: object[]}} */
    const { nodes } = response.body.data.organization.membersWithRole

    nodes.forEach(node => gitHubUsers.push({
      email: node.email,
      login: node.login
    }))

    this.log(`There are ${nodes.length} users on the ${organization} GitHub organization`)
   ...
   * @param {GitHubUsers[]} args.allUsers
   * @returns {String|undefined}
   */
  getUserEmail ({ author, allUsers }) {
